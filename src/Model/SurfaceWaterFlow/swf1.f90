...
module SwfModule
  ...
  use SwfMmrModule, only: SwfMmrType
  use SwfMctModule, only: SwfMctType
  ...
  type, extends(NumericalModelType) :: SwfModelType
    ...
    type(SwfMmrType), pointer :: mmr => null() !< muskingum routing package
    type(SwfMctType), pointer :: mct => null() !< muskingum-cunge-todini package
    ...
    integer(I4B), pointer :: indfw => null() !< unit number MMR
    integer(I4B), pointer :: inmmr => null() !< unit number MMR
    integer(I4B), pointer :: inmct => null() !< unit number MCT
  ...
  contains
    ...
    procedure :: model_solve => swf_solve !< routine for solving this model for the current time step
  ...
  data SWF_BASEPKG/'DISL6', 'DFW6 ', 'MMR6 ', 'MCT6 ', 'CXS6 ', & !  5
  ...
  subroutine allocate_scalars(this, modelname)
    ...
    call mem_allocate(this%inmct, 'INMCT', this%memoryPath)
  ...
  subroutine swf_ar(this)
    ...
    if (this%inmmr > 0) call this%mmr%mmr_ar()
    ...
    else if (this%inmmr > 0) then
      call this%oc%oc_ar('QOUTFLOW', this%mmr%qoutflow, this%dis, DNODATA)
    else if (this%inmct > 0) then
      call this%oc%oc_ar('QOUTFLOW', this%mct%qoutflow, this%dis, DNODATA)
    end if
  ...
  subroutine swf_rp(this)
    ...
    if (this%inmct > 0) call this%mct%mct_rp()
  ...
  subroutine swf_ad(this)
    ...
    if (this%inmct > 0) call this%mct%mct_ad(irestore)
  ...
  subroutine swf_solve(this)
    ...
    ! -- MMR/MCT Solve
    if (this%inmmr > 0) call this%mmr%mmr_solve(this%rhs)
    if (this%inmct > 0) call this%mct%mct_solve(this%rhs)

    ! -- return
    return
  end subroutine swf_solve
  ...
  subroutine swf_cq(this, icnvg, isuppress_output)
    ...
    if (this%inmct > 0) call this%mct%mct_cq(this%flowja)
  ...
  subroutine swf_bd(this, icnvg, isuppress_output)
    ...
    if (this%inmct > 0) call this%mct%mct_bd(isuppress_output, this%budget)
    do ip = 1, this%bndlist%Count()
      packobj => GetBndFromList(this%bndlist, ip)
  ...
  subroutine swf_ot_obs(this)
    ...
    ! -- Calculate and save mct observations
    if (this%inmct > 0) then
      call this%mct%mct_bd_obs()
      call this%mct%obs%obs_ot()
    end if
  ...
  subroutine swf_ot_flow(this, icbcfl, ibudfl, icbcun)
    ...
    if (this%indfw > 0) then
      ...
      call this%mct%mct_save_model_flows(this%flowja, icbcfl, icbcun)
    end if
    do ip = 1, this%bndlist%Count()
      packobj => GetBndFromList(this%bndlist, ip)
    ...
    ! -- Print SWF flows
    if (this%inmmr > 0) then
      call this%mmr%mmr_print_model_flows(ibudfl, this%flowja)
    else if (this%inmct > 0) then
      call this%mct%mct_print_model_flows(ibudfl, this%flowja)
    end if
    do ip = 1, this%bndlist%Count()
      packobj => GetBndFromList(this%bndlist, ip)
  ...
  subroutine swf_da(this)
    ...
    if (this%inmmr > 0) call this%mmr%mmr_da()
    if (this%inmct > 0) call this%mct%mct_da()
    ...
    call mem_deallocate(this%inmct)
...
! ------------------------------------------------------------------------------
  ...
  subroutine package_create(this, filtyp, ipakid, ipaknum, pakname, mempath, &
    ...
    case ('FLW6')
      call flw_create(packobj, ipakid, ipaknum, inunit, iout, this%name, &
                      pakname, mempath)
      !packobj%ictMemPath = create_mem_path(this%name, 'DFW')  ! doesn't work for MCT Package
      packobj%ictMemPath = ''
    case ('CHD6')
  ...
  subroutine ftype_check(this, indis)
    ...
    if (this%indfw == 0 .and. this%inmmr == 0 .and. this%inmct == 0) then
      write (errmsg, '(1x,a)') &
        'Either DFW6 or MMR6 or MCT6 Package must be specified.'
      call store_error(errmsg)
    end if
    if (this%inmmr > 0 .and. this%inmct > 0) then
      write (errmsg, '(1x,a)') &
        'Either MMR6 or MCT6 Package can be specified, but not both.'
      call store_error(errmsg)
    end if
  ...
  subroutine create_packages(this)
    ...
    use SwfMmrModule, only: mmr_cr
    use SwfMctModule, only: mct_cr
    ...
    character(len=LENMEMPATH) :: mempathmmr = ''
    character(len=LENMEMPATH) :: mempathmct = ''
    ...
    if (this%indfw > 0) then
      ...
      call mmr_cr(this%mmr, this%name, mempathmmr, this%inmmr, this%iout, &
                  this%dis)
    else if (this%inmct > 0) then
      call mct_cr(this%mct, this%name, mempathmct, this%inmct, this%iout, &
                  this%dis, this%incxs)
    end if
    if (this%insto > 0) then
