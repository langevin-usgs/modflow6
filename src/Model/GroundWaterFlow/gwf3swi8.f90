...
contains
  ...
  !<
  subroutine swi_ar(this, ibound, effective_top, effective_bot)
    ! -- dummy
    class(GwfSwiType) :: this
    integer(I4B), dimension(:), pointer, contiguous :: ibound !< model ibound
    real(DP), dimension(:), contiguous, pointer :: effective_top !< npf eff top
    real(DP), dimension(:), contiguous, pointer :: effective_bot !< npf eff bot
    ! -- local
    integer(I4B), pointer :: iforce_cond
    ...
    !
    ! -- set a temporary pointer into npf%iforce_cond and set
    !    to 1 so that condsat is always recalculated
    call mem_setptr(iforce_cond, 'IFORCE_COND', &
                    create_mem_path(this%name_model, 'NPF'))
    call mem_setptr(inewton, 'INEWTON', &
                    create_mem_path(this%name_model, 'NPF'))
    !
    ! todo: the logic here is not correct: condsat should be
    !       recalculated in all cases for icelltype == 0
    !       and for icelltype == 0 and 1 if not newton
    !       Or do we just suppport newton for swi?
    if (inewton == 0) then
      ! if not newton, then force condsat to be recalculated
      iforce_cond = 1
    else
      ! todo: if it is newton, then condsat should be recalculated
      !       for icelltype == 0
      ! Alternatively, if SWI is on and newton is on, should we force
      !  the user to specify icelltype == 1?
    end if
    !
    ! -- update zeta
